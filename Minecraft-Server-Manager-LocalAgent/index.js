...
const handleSetupModeExit = (message) => {
  console.log(`\n[SUCCESS] ${message}`);
  console.log(
    "El instalador continuará con la configuración de segundo plano...\n",
  );
  process.exit(0);
};
  ...
  try {
    ...
    daemon.onUnlink(async () => {
      ...
      try {
        ...
        console.log(
          "[System] Advertencia: No se pudo notificar a la nube de la desvinculación.",
        );
      }
      try {
    ...
    if (!agentToken) {
      ...
      if (isSetupMode)
        return handleSetupModeExit("Vinculación completada exitosamente.");
    }

    if (isSetupMode) {
      return handleSetupModeExit(
