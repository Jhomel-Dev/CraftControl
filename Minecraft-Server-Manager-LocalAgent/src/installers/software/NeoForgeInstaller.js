...
export default class NeoForgeInstaller {
  ...
  async resolveVersion(fullVersion) {
    ...
    const matches = [
      ...text.matchAll(
        new RegExp(`<version>(${neoPrefix}\\.[\\d\\.]+)</version>`, "g"),
      ),
    ];

    if (matches.length === 0)
      throw new Error(`No NeoForge builds found for Minecraft ${fullVersion}`);
  ...
  async install(fullVersion, dataDir, config) {
    ...
    await this.ensureNeoForgeIsDownloaded(
      mcVer,
      neoVer,
      resolvedFullVersion,
      neoDir,
    ...
    return {
      ...
      args: [
        "@user_jvm_args.txt",
        `@libraries/net/neoforged/neoforge/${neoVer}/${argsFileName}`,
      ],
    };
  }

  async ensureNeoForgeIsDownloaded(
    mcVer,
    neoVer,
    fullVersion,
    neoDir,
    ...
    const installerPath = path.join(
      this.jarsDir,
      `neoforge-installer-${fullVersion}.jar`,
    );
    ...
    const result = spawnSync(
      ...
      { cwd: neoDir, stdio: "ignore" },
    );

    if (result.error) {
      throw new Error(
        `Failed to start NeoForge installer: ${result.error.message}`,
      );
    }
    if (result.status !== 0) {
      throw new Error(
        `NeoForge installer failed with exit code ${result.status}`,
      );
    }
