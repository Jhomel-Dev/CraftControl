...
export default class SoftwareInstallerFactory {
  ...
  getInstaller(type) {
    switch (type) {
      case "VANILLA":
        return new VanillaInstaller(this.jarsDir);
      case "PAPER":
        return new PaperInstaller(this.jarsDir);
      case "FOLIA":
        const foliaInstaller = new PaperInstaller(this.jarsDir);
        foliaInstaller.install = (version, dir) =>
          PaperInstaller.prototype.install.call(
            foliaInstaller,
            version,
            dir,
            "folia",
      ...
      case "PURPUR":
        return new PurpurInstaller(this.jarsDir);
      case "FORGE":
        return new ForgeInstaller(this.jarsDir, this.javaInstaller);
      case "FABRIC":
        return new FabricInstaller(this.jarsDir, this.javaInstaller);
      case "NEOFORGE":
        return new NeoForgeInstaller(this.jarsDir, this.javaInstaller);
      default:
        throw new Error(
          `Software type ${type} is not supported or implemented yet.`,
        );
    }
  }
}
