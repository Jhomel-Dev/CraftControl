...
export default class PaperInstaller {
  ...
  async resolveVersion(software, fullVersion) {
    ...
    const versionsArr = Array.isArray(data.versions)
      ? data.versions
      : Object.values(data.versions).flat();
  ...
  async install(fullVersion, dataDir, software = "paper") {
    const { mcVer, buildVer } = await this.resolveVersion(
      software,
      fullVersion,
    );
    const res = await fetch(
      `https://fill.papermc.io/v3/projects/${software}/versions/${mcVer}`,
      {
        headers: { "User-Agent": "Minecraft-Server-Manager/1.0" },
      },
    );
    ...
    if (!res.ok)
      throw new Error(`Failed to fetch ${software} version metadata`);
    const data = await res.json();

    let latestBuild;
    if (buildVer) {
      if (!data.builds.includes(parseInt(buildVer)))
        throw new Error(`Build ${buildVer} not found for ${software} ${mcVer}`);
    ...
    if (!fs.existsSync(jarPath)) {
      const buildRes = await fetch(
        `https://fill.papermc.io/v3/projects/${software}/versions/${mcVer}/builds/${latestBuild}`,
        {
          headers: { "User-Agent": "Minecraft-Server-Manager/1.0" },
        },
      );
      ...
      const downloadUrl =
        buildData.downloads["server:default"]?.url ||
        buildData.downloads.application?.url ||
        Object.values(buildData.downloads)[0].url;
  ...
  cleanupOldJars(version, currentJarName, software) {
    try {
      ...
      for (const file of files) {
        if (
          file.startsWith(`${software}-${version}-b`) &&
          file !== currentJarName
        ) {
          try {
