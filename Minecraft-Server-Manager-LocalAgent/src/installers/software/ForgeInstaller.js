...
export default class ForgeInstaller {
  ...
  async resolveVersion(fullVersion) {
    ...
    const matches = [
      ...text.matchAll(
        new RegExp(`<version>(${fullVersion}-[\\d\\.]+)</version>`, "g"),
      ),
    ];

    if (matches.length === 0)
      throw new Error(`No Forge builds found for Minecraft ${fullVersion}`);
  ...
  async install(fullVersion, dataDir, config) {
    ...
    await this.ensureForgeIsDownloaded(
      mcVer,
      resolvedFullVersion,
      forgeDir,
      librariesDir,
    ...
    if (fs.existsSync(argsFile)) {
      return {
        ...
        args: [
          "@user_jvm_args.txt",
          `@libraries/net/minecraftforge/forge/${resolvedFullVersion}/${argsFileName}`,
        ],
      };
    }
  ...
  async ensureForgeIsDownloaded(
    ...
    const installerPath = path.join(
      this.jarsDir,
      `forge-installer-${fullVersion}.jar`,
    );
    ...
    const result = spawnSync(
      ...
      { cwd: forgeDir, stdio: "ignore" },
    );

    if (result.error) {
      throw new Error(
        `Failed to start Forge installer: ${result.error.message}`,
      );
    }
    if (result.status !== 0) {
