...
export default class PlayerStatsService {
  ...
  async getPlayers(serverId, onlineNames = []) {
    ...
    for (const file of files) {
      ...
      try {
        const playerData = await this.parsePlayerData(
          playersDir,
          file,
          baseDir,
          uuid,
  ...
  async buildUuidToNameMap(baseDir) {
    const uuidToName = {};
    try {
      const cacheStr = await fs.readFile(
        path.join(baseDir, "usercache.json"),
        "utf-8",
      );
  ...
  extractCustomName(tagObj) {
    ...
    try {
      ...
      return (
        nameData.text ||
        nameData.translate ||
        nameData.extra?.[0]?.text ||
        nameValue
      );
    } catch (e) {
      return nameValue.replace(/§[0-9a-fk-or]/gi, "");
    }
  }
