...
export default class NativeServerService extends EventEmitter {
  constructor() {
    ...
    this.softwareFactory = new SoftwareInstallerFactory(
      jarsDir,
      this.javaInstaller,
    );
  }
  ...
  async startMinecraftServer(config) {
    ...
    const javaExe = await this.javaInstaller.ensureJavaIsInstalled(
      config.version,
    );
    const softwareInstaller = this.softwareFactory.getInstaller(config.type);
    const softwareConfig = await softwareInstaller.install(
      config.version,
      config.dataDir,
      config,
    );
  ...
  validateHardwareLimits(config) {
    ...
    if (memMatch) {
      ...
      requestedBytes *=
        memMatch[2].toUpperCase() === "G" ? 1024 * 1024 * 1024 : 1024 * 1024;

      if (requestedBytes > os.freemem()) {
        const requestedStr =
          (requestedBytes / (1024 * 1024 * 1024)).toFixed(1) + "GB";
  ...
  handleCompatibilityMode(config, softwareConfig) {
    ...
    this.emit(
      "log",
      "[Compatibility Mode] Copying server locally to avoid conflicts...",
    );
  ...
  attachProcessListeners() {
    ...
    this.process.on("player_leave", (player) =>
