...
const generateRandomString = (length) => {
  ...
  for (let i = 0; i < length; i++) {
    result += ALPHANUMERIC_CHARS.charAt(
      crypto.randomInt(0, ALPHANUMERIC_CHARS.length),
    );
  }
  return result;
};
...
export const requestPairingCode = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ pin: pairingCode.pin, expiresAt: pairingCode.expiresAt });
  } catch (error) {
...
export const unlinkAgent = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Agent unlinked successfully" });
  } catch (error) {
    return res.status(500).json({ error: "InternalServerError" });
  }
};
...
export const getUpdateInfo = async (req, res) => {
  try {
    ...
    if (!process.env.AGENT_MIN_VERSION)
      console.warn("WARNING: AGENT_MIN_VERSION is not set in ENV!");
...
export const getLatestRelease = async (req, res) => {
  try {
    ...
    if (!process.env.AGENT_LATEST_VERSION)
      console.warn("WARNING: AGENT_LATEST_VERSION is not set in ENV!");
